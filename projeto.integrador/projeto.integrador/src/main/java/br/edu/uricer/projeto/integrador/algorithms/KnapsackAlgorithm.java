...
public class KnapsackAlgorithm {
    public Map<Integer, String> knapsackDynamicProgramming(
            int[] weightOfAvailableItems,
    ...
    //todo Implementar no futuro, pois assim faz mais sentido (removido o peso dos items)
    public Map<Integer, String> knapsackDynamicProgramming(
            int[] valuesOfAvailableItems,
            int availableItemsToPutInside,
            int targetValue,
            List<Item> itemList) {
        ...
        for (int i = 1; i <= availableItemsToPutInside; i++) {
            for (int j = 1; j <= targetValue; j++) {
                int valueWithoutItemI = m[i - 1][j];
                if (valuesOfAvailableItems[i - 1] > j) {
                    m[i][j] = valueWithoutItemI;
                } else {
                    int valueWithItemI = m[i - 1][j - valuesOfAvailableItems[i - 1]] + valuesOfAvailableItems[i - 1];
                    if (valueWithItemI > valueWithoutItemI) {
                        m[i][j] = valueWithItemI;
                        selected[i][j] = true;
                    } else {
